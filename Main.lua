...
function UILib:CreateSettingsTab(customName)
    ...
    self:Keybind(menuTab, menuSettings, 'Open key', 'x', function (state)
                        ...
                        if self._IsMouseWithinBounds(boxPosition, boxSize) then
                            if clickFrame then
                                local colorpickerCallback = function(newColor)
                                    sectionItem['value'] = {math.floor(newColor.R * 255), math.floor(newColor.G * 255), math.floor(newColor.B * 255)}

                                    if itemCallback then
                                        itemCallback(newColor)
                                    end
                                end

                                self:_SpawnColorpicker(Color3.fromRGB(unpack(sectionItem['value'])), sectionItem['label'], colorpickerCallback)
                            elseif ctxFrame then
...
print("=======================================")
print("Fling GUI v1.0")
print("Made by '@1i3t'")
print("UI Library made by @Nulare")
print("=======================================")
wait(1)
...
local extraSection = ui:Section(generalTab, 'Extra')

local settingsTab, settingsSection, themingSection = ui:CreateSettingsTab('Settings')
task.defer(function()
    local themingItems = ui._tree._tabs[#ui._tree._tabs]._sections[2]
    local colorAccent = themingItems._items[2]
    local colorBase = themingItems._items[3]
    local colorInnerStroke = themingItems._items[4]
    local colorOuterStroke = themingItems._items[5]
...
local function isBasePart(obj)
    ...
    return t == "Part" or t == "MeshPart" or t == "WedgePart" or t == "CornerWedgePart" or t == "TrussPart" or t == "UnionOperation"
end
...
local function disableModelCollision(model)
    ...
    for i = 1, #list do
        local obj = list[i]
        if obj and isBasePart(obj) and obj.CanCollide ~= nil then
...
local function startFling()
    ...
    spawn(function()
        while flingRunning and flingEnabled and flingToggled do
            local currentHrp = getCharacter()
...
ui:Checkbox(generalTab, mainSection, 'Enable Fling', false, function(state)
    flingEnabled = state
    if not state then
        flingToggled = false
        stopFling()
...
ui:Button(settingsTab, settingsSection, 'Unload GUI', function()
    running = false
    flingRunning = false
    noCollisionLoop = false
    wait(0.1)
    ui:Destroy()
end)

local running = true
local noCollisionLoop = false
    ...
    while running do
        if ui and ui.Step then
            ...
            if noCollision and not noCollisionLoop then
                noCollisionLoop = true
                spawn(function()
                    while noCollision and running do
                        disableOthers()
                        task.wait(1)
                    end
                    noCollisionLoop = false
